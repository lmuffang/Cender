...
def test_create_template(client, test_user):
    ...
    response = client.post(f"/users/{test_user.id}/template", json={"content": template_content, "subject": template_subject})
...
def test_update_template(client, test_user, test_template):
    ...
    response = client.post(f"/users/{test_user.id}/template", json={"content": new_content, "subject": new_subject})
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_template_user_not_found(client):
    ...
    response = client.post("/users/99999/template", json={"content": "Test", "subject": "Test Subject"})
    assert response.status_code == status.HTTP_404_NOT_FOUND
